-- 全新 volume 五账号初始化（方案 §6.1 / 附录 A.1 #2 #3，Community §4.2，Study 方案 §21）。
-- 仅由 docker-entrypoint-initdb.d 在空数据目录首次启动时自动执行（此时引导超级用户
-- 为 postgres，memory / auth / conversation / community / study 尚未存在）。
...
ALTER ROLE study WITH LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION PASSWORD 'study';
