...
def write_chunk(raw_dir: Path, *, page_count: int, image_name: str, text: str, page_offset: int = 0) -> None:
    ...
    (raw_dir / "part_content_list.json").write_text(json.dumps(content, ensure_ascii=False), encoding="utf-8")
    v2 = [[
        {"type": "paragraph", "content": {"paragraph_content": [{"type": "text", "content": text}]}, "bbox": [1, 1, 10, 10]},
        {"type": "equation_interline", "content": {"math_content": "x_1 + 2 = 3", "math_type": "latex", "image_source": {"path": f"images/{image_name}"}}, "bbox": [1, 10, 20, 20]},
        {"type": "table", "content": {"html": "<table><tr><td>A</td></tr></table>"}, "bbox": [1, 20, 30, 30]},
    ]]
    while len(v2) < page_count:
        v2.append([])
    (raw_dir / "part_content_list_v2.json").write_text(json.dumps(v2, ensure_ascii=False), encoding="utf-8")
...
class MergeTest(unittest.TestCase):
    ...
    def test_validate_chunk_result_maps_page_idx_and_checks_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            chunk = {"book_id": "01_book", "book_name": "书", "chunk_id": "c1", "page_start": 5, "page_end": 6}
    ...
    def test_merge_book_keeps_page_order_and_renames_images(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for number, start in enumerate((1, 3), start=1):
                ...
                write_chunk(chunk_dir / "raw", page_count=2, image_name="same.jpg", text=f"书页{start}")
                chunks.append({"chunk_id": f"c{number}", "chunk_dir": str(chunk_dir), "page_start": start, "page_end": start + 1})
            book = {"book_id": "01_book", "book_name": "书一", "source_filename": "book.pdf", "page_count": 4, "chunks": chunks}
            summary = merge_book(book, output)
            self.assertEqual(summary["status"], "merged")
            records = [json.loads(line) for line in (book_dir / "content_list.jsonl").read_text().splitlines()]
            self.assertEqual([r["source_page"] for r in records if r["element_type"] == "text"], [1, 3])
    ...
    def test_merge_all_books_does_not_mix_books(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for book_id, text in (("01_a", "甲书"), ("02_b", "乙书")):
                ...
                books.append({"book_id": book_id, "book_name": book_id, "source_filename": f"{book_id}.pdf", "page_count": 1, "chunks": [{"chunk_id": "c1", "chunk_dir": str(chunk_dir), "page_start": 1, "page_end": 1}]})
