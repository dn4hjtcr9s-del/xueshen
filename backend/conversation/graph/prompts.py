...
（流式输出模式）回答正文输出完成后，请在最后单独一行输出结构收尾标记：
<followups>["追问1","追问2","追问3"]</followups>
- 该标记只用于服务端解析追问建议，不属于回答正文，必须出现在正文之后；
- 最多 3 条追问；没有追问时输出 <followups>[]</followups>；
- 保持 <followups> 标记本身为一行完整 JSON，不要使用代码围栏包裹。"""

#: 记忆工具模式引导词（memory-rebuild §2.4 D3③：对齐 codex read_path.md 的
#: "quick memory pass"：只在需要用户学习历史/偏好时查一次，先 search 定位再 read 下沉）。
#: 仅在网关被显式传入 tools 时追加，tools=None 的既有路径不拼接本段。
ANSWER_MEMORY_TOOLS_INSTRUCTION = """
（记忆工具模式）本轮为你提供了长期记忆工具 memory_search / memory_read：
- 当问题涉及用户的学习历史、已掌握或未掌握的知识点、稳定偏好，或需要"你之前提到…"式
  个性化时，先调用 memory_search 做**一次**快速关键词检索定位条目，再按需调用
  memory_read 读取正文片段；不确定就先查一次，但不要反复检索同一内容；
- 必须"先 search 定位、再 read 下沉"：search 只返回条目标题/描述/关键词，不含正文，
  不要凭标题猜测正文内容，也不要把 search 结果当作已读过的正文；
- 长期记忆是用户历史的推断且可能陈旧：可用于个性化说明，但不得用它补充教材事实，
  也不得声称记忆里没有的内容；工具结果为空或不足时，直接说明无法从当前学习记录判断；
- 只能访问当前用户自己的记忆，不要请求或推测其它用户的数据；
- 工具调用预算有限（每轮最多 6 次）：用完就基于已有信息作答，不要为"再确认一次"重复调用。"""
